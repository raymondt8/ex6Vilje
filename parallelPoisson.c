...
void parallelPoisson(int problemSize,MPI_Comm comm)
{
	...
	Vector fstBuffer = createVector(fstBufferSize*ompThreads);
	fstBuffer->localSize = ompThreads;
	fstBuffer->globalSize = fstBufferSize;
	#else
	...
	{
		printf("p*t = %i * %i\n",local_b->commSize, fstBuffer->localSize);
		printf("Runtime: %fs\n",endTime-startTime);
	}
...
void fastSineTransform(ColumnMatrix localMatrix, Vector fstBuffer)
{
	...
	for(int column=0;column<localMatrix->localSize;column++)
	{
		fst_(&localMatrix->data[column*localMatrix->globalSize], &problemSize, &fstBuffer->data[omp_get_thread_num()*fstBuffer->globalSize], &fstBuffer->localSize);
	}
	#else
	for(int column=0;column<localMatrix->localSize;column++)
	{
...
void fastSineTransformInv(ColumnMatrix localMatrix, Vector fstBuffer)
{
	...
	for(int column=0;column<localMatrix->localSize;column++)
	{	
		fstinv_(&localMatrix->data[column*localMatrix->globalSize], &problemSize, &fstBuffer->data[omp_get_thread_num()*fstBuffer->globalSize], &fstBuffer->localSize);
	}
	#else
	for(int column=0;column<localMatrix->localSize;column++)
	{	
